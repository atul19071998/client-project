import React from "react";
import "../Componentcss/Navbar.css";
const Navbar = () => {
  return (
    <>
    <div
      className="navbar-wrapper "
      style={{
        position: "fixed",
        top: "0px",
        backgroundColor: "white",
        zIndex: "100",
      }}
    >
      <div className="navbar-wrap">
        <div className="navbar-left">
          <img
            src="data:image/png;base64,"
            alt="..."
          />
        </div>
        <div className="navbar-right">
          
          <div className="navbar-right1">
            <div class="dropdown">
              <div className="dropdowns">
                <i class="fa-solid fa-braille"></i>
                <button class="dropbtn">Categories </button>
                <i class="fa-solid fa-chevron-down"></i>
              </div>
              <div class="dropdown-content">
                <a href="#">English Book</a>
                <a href="#">Math Book</a>
                <a href="#">Story Book</a>
              </div>
            </div>
          </div>
          <div className="navbar-right2">
            <div class="dropdown">
              <div className="dropdowns">
                <button class="dropbtn">Home</button>
                <i class="fa-solid fa-chevron-down"></i>
              </div>
              <div class="dropdown-content">
                <a href="#">Home One</a>
                <a href="#">Home Two</a>
                <a href="#">Home Three</a>
              </div>
            </div>
          </div>
          <div className="navbar-right3">
            <div class="dropdown">
              <div className="dropdowns">
                <button class="dropbtn">Pages</button>
                <i class="fa-solid fa-chevron-down"></i>
              </div>
              <div class="dropdown-content">
                <a href="#">About</a>
                <a href="#">Instructor</a>
                <a href="#">Profile</a>
                <a href="#">Login</a>
                <a href="#">Signup</a>
                <a href="#">Events</a>
              </div>
            </div>
          </div>
          <div className="navbar-right4">
            <div class="dropdown">
              <div className="dropdowns">
                <button class="dropbtn">Courses</button>
                <i class="fa-solid fa-chevron-down"></i>
              </div>
              <div class="dropdown-content">
                <a href="#">Course</a>
                <a href="#">Right Sidebar</a>
                <a href="#">Course Single</a>
              </div>
            </div>
          </div>
          <div className="navbar-right5">
            <div class="dropdown">
              <div className="dropdowns">
                <button class="dropbtn">Blog</button>
                <i class="fa-solid fa-chevron-down"></i>
              </div>
              <div class="dropdown-content">
                <a href="#">Blog</a>
                <a href="#">Blog Simple</a>
              </div>
            </div>
          </div>
          <div className="navbar-right6">
            <div class="dropdown">
              <button class="dropbtn">Contact</button>
            </div>
          </div>
          <div className="navbar-right7">
            <div className="navbar-center">
              <div className="navbar-search-l">Search Course</div>
              <div className="navbar-search-r">
                <i class="fa-solid fa-magnifying-glass"></i>
              </div>
            </div>
          </div>
        </div>
      </div>
       
   
    </div>
    <div class="nav">
  <input type="checkbox" id="nav-check"/>
  <div class="nav-header">
    <div class="nav-title">
      <img src="data:image/png;base64," alt=".." />
    </div>
  </div>
  <div class="nav-btn">
    <label for="nav-check">
      <span></span>
      <span></span>
      <span></span>
    </label>
  </div>
  
  <div class="nav-links">
    <a href="//github.io/jo_geek" target="_blank">Home</a>
    <a href="http://stackoverflow.com/users/4084003/" target="_blank">Pages</a>
    <a href="https://in.linkedin.com/in/jonesvinothjoseph" target="_blank">Courses</a>
    <a href="https://codepen.io/jo_Geek/" target="_blank">Blog</a>
    <a href="https://jsfiddle.net/user/jo_Geek/" target="_blank">Contact</a>
  </div>
</div>
   </>
  );
};

export default Navbar;
